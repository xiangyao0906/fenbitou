 ...
 * 保利卫视本地视频播放
 * */
public class BLLocalVideoActivity extends FragmentActivity {
    ...
    private void initView() {
        ...
        videoView.setOnVideoTimeoutListener(new IPolyvOnVideoTimeoutListener() {
            ...
            public void onBufferTimeout(int timeoutSecond, int times) {//在一个缓冲里，每超过设置的timeoutSecond都会回调一次
                Toast.makeText(BLLocalVideoActivity.this, "视频加载速度缓慢，请切换到低清晰度的视频或调整网络", Toast.LENGTH_LONG).show();
            }
        });
        ...
        videoView.setOnVideoStatusListener(new IPolyvOnVideoStatusListener() {
            ...
            public void onStatus(int status) {
                if (status < 60) {
                    Toast.makeText(BLLocalVideoActivity.this, "状态错误 " + status, Toast.LENGTH_SHORT).show();
                } else {
...
//        });
        ...
        videoView.setOnGestureLeftUpListener(new IPolyvOnGestureLeftUpListener() {
            ...
            public void callback(boolean start, boolean end) {
                Log.d(TAG, String.format("LeftUp %b %b brightness %d", start, end, videoView.getBrightness(BLLocalVideoActivity.this)));
                if(mediaController.isLocked()){
                    return;
                }

                int brightness = videoView.getBrightness(BLLocalVideoActivity.this) + 5;
                if (brightness > 100) {
                    brightness = 100;
                }

                videoView.setBrightness(BLLocalVideoActivity.this, brightness);
        ...
        videoView.setOnGestureLeftDownListener(new IPolyvOnGestureLeftDownListener() {
            ...
            @Override
            public void callback(boolean start, boolean end) {
                Log.d(TAG, String.format("LeftDown %b %b brightness %d", start, end, videoView.getBrightness(BLLocalVideoActivity.this)));
                if(mediaController.isLocked()){
                    return;
                }
                int brightness = videoView.getBrightness(BLLocalVideoActivity.this) - 5;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            ...
            if (PolyvScreenUtils.isLandscape(this) && mediaController != null&&!isMustFromLocal) {
                mediaController.changeToPortrait();
                return true;
            }
    ...
    public static Intent newIntent(Context context, PlayMode playMode, String vid, int bitrate, boolean startNow,
        ...
        Intent intent = new Intent(context, BLLocalVideoActivity.class);
